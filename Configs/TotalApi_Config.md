...
Данные параметры должны быть указаны в файле `appconfig.json`/`appconfig.yml`, которые находятся в каталоге **TotalApi**.
Дополнительно данные могут быть загружены из файлов `*.json`/`*.yml`, находящихся в подкаталоге **.configs** каталоге **TotalApi**.
...
ВАЖНО! Перед изменением конфига - выполните деинсталляцию.

Общие параметры подсистем
-------------------------
    
    serviceHost                              - адрес и (опционально) порт и протокол подключения к сервису;
        [protocol:]//[host][:порт]             если протокол не указан используется `http`;
